...
function parsePriceList(priceList: string) {
	...
	if (header !== "sale id\tgroup id\tname\tprice\tvalue price\tdiscount tag\tincludes\ttag\tsale start\tsale end\tsale name") {
	...
	for (let i = 0; i < data.length; i += 3) {
		const [saleId, groupId, _3, _4, _5, _6, _7, _8, saleStart, saleEnd, saleName] = data[i].split("\t");
		...
		for (let j = i; j < i + 3; j++) {
			const [_1, _2, name, price, valuePrice, discountTag, includes, tag, _9, _10, _11] = data[j].split("\t");
